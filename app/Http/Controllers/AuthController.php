...
class AuthController extends Controller
{
    public function login()
    {
        ...
        if (auth()->user()) {
            ...
            } else if (auth()->user()->role == 'penyetuju') {
    ...
    public function postlogin(Request $request)
    {
        ...
        if (Auth::attempt($cre)) {
            ...
            $UserChart = new grafik;
        $UserChart->labels(['Jan', 'Feb', 'Mar']);
        $UserChart->dataset('Users by trimester', 'line', [10, 25, 13]);
        
            // dd($UserChart);
    
            return view('/dashboard', compact('data_admin','UserChart'));
        }
